...
func (s *Server) registerRoutes(h *hertzserver.Hertz) {
	...
	api := h.Group("/api")
	{
		...
		// 资料卡片索引系统 (material-index)
		api.GET("/material-index/search", apiHandlers.HandleMaterialIndexSearch)
		api.GET("/material-index/stats", apiHandlers.HandleMaterialIndexStats)
		api.GET("/material-index/card/:id", apiHandlers.HandleMaterialIndexCard)
		api.GET("/material-index/templates", apiHandlers.HandleMaterialIndexTemplates)
		api.POST("/material-index/generate", apiHandlers.HandleMaterialIndexGenerate)
		api.POST("/material-index/rebuild", apiHandlers.HandleMaterialIndexRebuild)
		api.POST("/material-index/import", apiHandlers.HandleMaterialIndexImport)
		api.GET("/material-index/imports", apiHandlers.HandleMaterialIndexImports)
		api.DELETE("/material-index/imports", apiHandlers.HandleMaterialIndexImportDelete)
		api.GET("/material-index/workspace-search", apiHandlers.HandleMaterialIndexWorkspaceSearch)
		...
		api.POST("/diagrams/generate", apiHandlers.HandleDiagramGenerate)
